...
public class SimpleNode implements Node {
		...
		case YalToJvmTreeConstants.JJTTERM:
			if (Op != "" || ID != null) {
				...
				if (temp.Op.equals("+") || temp.Op.equals("-"))
					System.out.println(newPrefix(prefix, true) + "[ " + Op + ID + " ]");
				else if (temp.Op.equals("*") || temp.Op.equals("/") || temp.Op.equals("<<") || temp.Op.equals(">>")
						|| temp.Op.equals(">>>"))
					System.out.println(newPrefix(prefix, true) + "[ " + Op + ID + " ]");
	...
	public void getAttributes(String prefix) {
		for (int i = 0; i < children.length; i++) {
			SimpleNode n = (SimpleNode) children[i];
			if (n != null) {
				if (n.id == YalToJvmTreeConstants.JJTFUNCTION) // function ()
					break;
				if (n.id == YalToJvmTreeConstants.JJTASSIGNEMENT) { // "=" OPERATOR FOUND!

					SimpleNode c1 = (SimpleNode) n.children[0];

					if (c1.id == YalToJvmTreeConstants.JJTGLOBAL) { // "c1.ID =" FOUND!
						String nameOp = c1.ID;
						Variable varOp = new Variable(nameOp);

						SimpleNode c2 = (SimpleNode) c1.children[0];

						if (c2.id == YalToJvmTreeConstants.JJTSCALAR) {
							// "c1.ID = scalar" FOUND!
							int value = Integer.parseInt(c2.ID);
							varOp = new Scalar(nameOp, value);
						} else if (c2.id == YalToJvmTreeConstants.JJTARRAY) {
							// "c1.ID = array" FOUND!
							int size = Integer.parseInt(c2.ID);
							varOp = new Array(nameOp, size, 00100100);
						} else if (c2.id == YalToJvmTreeConstants.JJTGLOBAL) {
							Variable operand = new Variable(c2.ID);
							if (YalToJvm.getModule().checkGlobalVariableID(operand)) { // VAR is initialized
								if(YalToJvm.getModule().getGlobalVariable(operand) instanceof Scalar){
									int value = ((Scalar) YalToJvm.getModule().getGlobalVariable(operand)).getValue();
									varOp = new Scalar(nameOp, value);
								} else if(YalToJvm.getModule().getGlobalVariable(operand) instanceof Array){
									int size = ((Array) YalToJvm.getModule().getGlobalVariable(operand)).getSize();
									varOp = new Array(nameOp, size, 00100100);
								}
								else{
									YalToJvm.semanticErrorMessages.add("Variable " + c2.ID + "is not initialized!");
								}
							} else {
								YalToJvm.semanticErrorMessages.add("Variable " + c2.ID + "is not initialized!");
							}
						...
						/* REDEFINIR ATRIBUTO ??
						 * if
						 * (YalToJvm.getModule().checkGlobalVariableID(varOp))
						 * YalToJvm.getModule().replaceGlobalVariable(varOp);
						 * else YalToJvm.getModule().addGlobalVariable(varOp);
						 */
						YalToJvm.getModule().addGlobalVariable(varOp);
					}
				} else if (n.children[0] == null && n.id == YalToJvmTreeConstants.JJTGLOBAL) { // Declaration without initialization
					String nameOnly = n.ID;
					Variable var = new Variable(nameOnly);

					if (!YalToJvm.getModule().addGlobalVariable(var)) {
						YalToJvm.semanticErrorMessages.add("Variable " + nameOnly + "already exists");
					}
				}
			}
		}
